...
function checkJSON(xmlDOM, val) {
    ...
    for (i = 0; i<row.length; i++) {
        if (val == row[i].value1 ) {
            $(".phone_hide").append("<div class='box" + val + " span"+idx+"'><input type='checkbox' class='new_box "+val+"' name='boxbox' value='" + row[i].cal + "'><span class='span"+idx+"'>" + row[i].name + "</span></div>");
            idx++;
        }
    }
...
function imgJSON(xmlDOM, val,two){
    ...
    if ($('.phone_img_hide  div').hasClass("box" + val)) {
        $('.phone_img.box' + val).remove();
        return;
    };

    for(i=0;i<row.length;i++){
        if(val == row[i].cal  && two ==row[i].value1){
            $(".phone_img_hide").append("<div class='phone_img box"+two+"'><img src='"+row[i].image+"' style='width:200px;height:200px'/></div>");
            return;
        }
    }
}
