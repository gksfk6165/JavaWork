...
public class File11Main {
	public static void main(String[] args) {
		...
		try (FileWriter fw = new FileWriter(dst); FileReader fr = new FileReader(src);) {
			String str = "안녕하세요";
			char[] charArr = { 'J', 'A', 'V', 'A' };

			fw.write(str); // 저장은 시스템인코딩 상태에따라저장 //utf-8경우 한글은 한글자당 3byte 영문은 1byte
			fw.write(charArr);
			fw.flush(); // write()호출후에는 flush()하여 출력버퍼의 데이터가 완전히 출력되도록 한다.
			
			char[] buff=new char[100];
			int charsRead=0; //읽어드릴 문자의 개수
			charsRead=fr.read(buff);
			for (int i = 0; i < charsRead; i++) {
				System.out.print(buff[i]);
			}
			System.out.println();
			System.out.println("읽은문자개수"+charsRead);
		} catch (IOException e) {
