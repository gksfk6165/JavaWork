...
// [동작 설계]
// 1. 이름(name) 과 전화번호(phoneNum) 과 (email) 값이 주어지면 전화번호 데이터(PhonebookModel) 를 생성하여 저장,  
...
//		주어진 이름(name) 과 전화번호(phoneNum) 과 (email) 값 으로 변경.  성공하면 1, 실패하면 0 리턴
...
public interface PhoneBookController {
	...
	public abstract int updatByUid(int uid,String name,String phoneNum,String memo);
	public abstract int deleteByUid(int uid);
	public abstract PhonebookModel choiceByUid(int uid);
